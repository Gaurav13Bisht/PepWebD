...
browserPromise.then(function (browser) {
    ...
    let pageOpenPromise = tab.goto("https://www.imdb.com/" + data[0]);
    return pageOpenPromise;
}).then(function () {
    let list = tab.$$(".lister-item-header a");
    return list;
}).then(function (data) {
    let movurls = [];
    for (let i = 0; i < data.length; i++) {
        let listfetchpromise = tab.evaluate(function (ele) {
            return ele.getAttribute("href");
        }, data[i]);
        movurls.push(listfetchpromise);
    }
    return Promise.all(movurls);
}).then(function (data) {
    let pageOpenPromise = tab.goto("https://www.imdb.com" + data[1]);
    return pageOpenPromise;
}).then(function () {
    let waitPromise = tab.waitForSelector("div[class = 'title_wrapper']", { visible: true });
    return waitPromise;
}).then(function () {
    let list = tab.$$("div[class = 'title_wrapper']");
    return list;
}).then(function (data) {
    console.log(data);
